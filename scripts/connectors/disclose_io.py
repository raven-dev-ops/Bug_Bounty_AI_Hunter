...
DEFAULT_URLS = ["https://raw.githubusercontent.com/disclose/diodb/main/programs.json"]
ENV_VAR = "BBHAI_DISCLOSE_IO_URL"
...
def _extract_scope(record):
    scope_block = first_value(record, ("scope", "targets"))
    ...
    if isinstance(scope_block, dict):
        if not in_scope:
            in_scope = first_value(scope_block, ("in_scope", "inScope"))
        if not out_scope:
            out_scope = first_value(scope_block, ("out_of_scope", "outScope"))
    elif isinstance(scope_block, list) and not in_scope:
        in_scope = scope_block
...
def _build_program(record, source):
    name = first_value(record, ("vendor", "name", "program", "organization"))
    handle = first_value(record, ("handle", "slug", "program_handle", "code"))
    platform = first_value(record, ("platform", "platform_name"))
    policy_url = first_value(record, ("policy_url", "policy", "url", "policyUrl"))
...
class DiscloseIoConnector(Connector):
    name = "disclose-io"
