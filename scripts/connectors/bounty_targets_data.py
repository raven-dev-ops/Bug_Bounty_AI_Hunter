...
DEFAULT_URLS = [
    "https://raw.githubusercontent.com/arkadiyt/bounty-targets-data/master/data/targets.json"
]
ENV_VAR = "BBHAI_BOUNTY_TARGETS_URL"
...
def _extract_scope(record):
    ...
    targets = first_value(record, ("targets", "scope"))
...
def _build_program(record, source):
    name = first_value(record, ("name", "program_name", "program", "vendor"))
    handle = first_value(record, ("handle", "slug", "program_handle", "code"))
    platform = first_value(record, ("platform", "platform_name", "platform_handle"))
    policy_url = first_value(record, ("policy_url", "program_url", "url", "programUrl"))
    reward_value = first_value(record, ("reward", "rewards", "bounty", "bounty_range"))
...
class BountyTargetsDataConnector(Connector):
...
register(BountyTargetsDataConnector())
